...
rownames(counts1_matrix) = counts1$Geneid
...
rownames(counts2_matrix) = counts2$Geneid
...
counts_matrix_filtered = counts1_matrix[keep,]
```
Take the RPKM.
```{r}
...
write.table(results, paste0("~/Downloads/duwaerts_", names(table(key1$SAMPLE.TYPE)), "_de_", curr_date, ".xls"), row.names=rownames(counts_matrix_filtered), col.names=NA, sep="\t")
...
write.table(topGOResults2, file = paste0("~/Downloads/duwaerts_", names(table(key1$SAMPLE.TYPE)), "_topGO_", curr_date, ".xls"), sep="\t",col.names=NA)
...
```{r}
#what are the genes that are different in the outlier sample ribosomal?
