...
annotation <- data.frame( DiseaseStatus=DiseaseStatus)
rownames(annotation) <- matrix(unlist(strsplit(colnames(fpkm_matrix), "_")), nc=3, byrow=T)[,2]
pheatmap(t(cbind( t(gapdh_fpkm), qPCR_table2[matchrow, "mean_GAPDH"])) ,scale="row", annotation=annotation)

pheatmap(t(cbind( t(gapdh_fpkm))) , annotation=annotation)
