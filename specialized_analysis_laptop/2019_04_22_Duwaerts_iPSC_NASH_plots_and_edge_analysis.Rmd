---
Title: "Duwaerts RNA-seq human iPSC lines"
...
```{r}
filename2 = "~/Downloads/2019_04_25_Duwaerts_analysis_plots/readcounts_iPSC.txt"
...
key = read.table("~/Downloads/2019_04_25_Duwaerts_analysis_plots/metadata_Duwaerts_iPSC.txt", header = T, stringsAsFactors = F, fill=T, sep="\t")
head(key)
...
```{r}
OutlierStatus = factor(colnames(counts_matrix_filtered) %in% c("iPS_20_NASH", "iPS_22_NASH", "iPS_54_CONTROL"), levels = c("FALSE","TRUE"))
...
fit <- glmFit(y_for_DE_outliers, design=design_matrix_outliers, dispersion=y_for_DE_outliers$trended.dispersion)
```

Outlier vs everyone else
```{r}
lrt <- glmLRT(fit, coef = "OutlierStatusTRUE")
topTags(lrt)
```

```{r}
counts_matrix_filtered_sansoutliers = counts_matrix_filtered[,!colnames(counts_matrix_filtered) %in% c("iPS_20_NASH", "iPS_22_NASH", "iPS_54_CONTROL")]
