...
```{r}
#write.table(all_fisher_tests, "Downloads/all_fisher_tests.xls", sep="\t", quote=F, row.names = F, col.names = c("TF", "TF_motif_num", "fisher_test_pvalue", "fg_hit", "fg_no_hit", "bg_hit", "bg_no_hit"))
#matches_fg_per_gene$Hnf4a[[1]]

#save(matches_fg_per_gene, matches_bg_per_gene, tests_per_gene, file = "Downloads/fisher_first_attempt.Robj")
```
...
```{r}
save(matches_fg_per_gene, matches_bg_per_gene, tests_per_gene, file = "Downloads/fisher_second_attempt.Robj")
write.table(all_fisher_tests, "Downloads/all_fisher_tests_v2.xls", sep="\t", quote=F, row.names = F, col.names = c("TF", "TF_motif_num", "fisher_test_pvalue", "fg_hit", "fg_no_hit", "bg_hit", "bg_no_hit"))
```

```{r}
#list of motif counts per promoter
for (gene in matches_fg_per_gene) {
   for (motif in matches_fg_per_gene[[gene]] ) {
     counts = matches_fg_per_gene[[gene]][[motif]]
     print (counts)
   }
}
...
```{r}
length(myseqs_foreground) + length(myseqs_notforeground)
