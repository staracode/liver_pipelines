...
---
---
title: "Chen lab RNA-seq version 2 with new samples"
...
```{r}
filename = "/Users/tfriedrich/Documents/chen/readcounts.txt"
counts1_old = read.table(filename, header = T,stringsAsFactors = F) 
rownames(counts1_old) = counts1_old$Geneid
filename2 = "/Users/tfriedrich/Documents/chen/readcounts_newsamples.txt"
counts1_new = read.table(filename2, header=T, stringsAsFactors = F)                             
rownames(counts1_new) = counts1_new$Geneid   
length(which(counts1_old$Geneid==counts1_new$Geneid))
counts1 = data.frame(cbind(counts1_old, counts1_new[,7:ncol(counts1_new)]))      
dim(counts1)
```
Remove metadata (Gene, coordinates, etc)
```{r}
counts2 = data.matrix(counts1[,7:ncol(counts1)])
...
```{r}
groupNamesPerSample = c(matrix (unlist(strsplit(colnames(counts4[,1:20]), "_")), byrow=T, ncol=4) [,1], colnames(counts4[,21:24]))
...
subjectgroup = c(matrix (unlist(strsplit(colnames(counts4[,1:20]), "_")), byrow=T, ncol=4) [,1], colnames(counts4[,21:24]))
...
Again S449 technical replicate showing batch effects. 
...
colnames(counts5) = c(unlist(lapply (strsplit(sub("counts3....", "" ,colnames(counts5[,1:16])) , "_"), `[[`, 1)), sub("counts3.", "", colnames(counts5[, 17:20])))
colnames(counts5) = gsub("\\.", "", colnames(counts5))
rownames(counts5) = rownames(counts3)
```
```{r}
library(ggplot2)
library(tidyr)
ggplot(gather((counts5[,1:5]), cols, value), aes(x = value)) + 
...
```{r}
colnames(counts5) = paste0(colnames(counts5), c(rep("_1", 16), rep("_2", 4)))
...
fpkm_log_matrix2 = rpkm (y_for_DE, gene.length=counts1[keep,"Length"], log = FALSE)
...
Heatmap of DE genes.  
```{r}
pheatmap(fpkm_log_matrix2[order(lrt_T_table$PValue, decreasing = F)[1:20],c(colnames(fpkm_log_matrix2)[grep("SC", colnames(fpkm_log_matrix2))],"FocusT_1", "PLCT_1", "S449T_1", "S475T_1", "S449T_2")], cluster_rows=FALSE, cluster_cols=FALSE)

pheatmap(fpkm_log_matrix2[order(lrt_Y_table$PValue, decreasing = F)[1:20],c(colnames(fpkm_log_matrix2)[grep("SC", colnames(fpkm_log_matrix2))],"FocusY_1", "PLCY_1", "S449Y_1", "S475Y_1", "S449Y_2")], cluster_rows=FALSE, cluster_cols=FALSE)
...
pheatmap(mean_values[order(lrt_T_table$PValue, decreasing = F)[1:20],c(colnames(fpkm_log_matrix2)[grep("SC", colnames(fpkm_log_matrix2))],"FocusT_1", "PLCT_1", "S449T_1", "S475T_1", "S449T_2")], cluster_rows=FALSE, cluster_cols=FALSE)
pheatmap(mean_values[order(lrt_Y_table$PValue, decreasing = F)[1:20],c(colnames(fpkm_log_matrix2)[grep("SC", colnames(fpkm_log_matrix2))],"FocusY_1", "PLCY_1", "S449Y_1", "S475Y_1", "S449Y_2")], cluster_rows=FALSE, cluster_cols=FALSE)
